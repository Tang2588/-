...
├── 规模因子_交付版/              Size
│
├── 四因子计算交付说明.tex        四个因子的完整说明（公式、清洗、验证）
...
实测北交所股票的有效因子覆盖率，切换前（2025-10-09 之前）与切换后对比：

| 因子 | 切换前 | 切换后 | 变化 |
|---|---:|---:|---|
| EP | 88.66% | 96.18% | 上升，映射生效 |
| PB | 92.07% | 80.63% | 掉 11.4 个百分点 |
| **ROE** | 68.43% | **3.49%** | **几乎完全失效** |
| Size | 99.69% | 99.73% | 不受影响（只用行情，不连接财报） |
